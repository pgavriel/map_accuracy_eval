...
def load_config(config_path=None,verbose=True):
    """Loads anf validates configuration from a JSON file one directory above the script."""
    ...
    required_keys = {
        "root_directory": str,
        "ground_truth_file": (str, type(None)),  # Allows None
        "eval_file": (str, type(None)),  # Allows None
        "event_name": str,
        "note": str,
...
if __name__ == "__main__":
    ...
    eval_map_file = os.path.basename(eval_map_path).split(".")[0]
    output_incomplete_path = join(os.path.dirname(eval_map_path),eval_map_file+"_missing_points.ply")
    ...
    if log_results:
        headers = ["Timestamp","Event","Note","GT File","Eval File",
                   ...
                   "BB Extent","BB Volume","BB Density","CH Volume","CH Density",
                    "Dist GT->EV","Dist EV->GT","Dist Avg (Chamfer Distance)","Completeness","CompDist Threshold"]
        bb_extent = [eval_info["Bound Extent X"],eval_info["Bound Extent Y"],eval_info["Bound Extent Z"]]
        
        log_data = [util.timestamp(),event_name,note,os.path.basename(ground_truth_path),os.path.basename(eval_map_path),
