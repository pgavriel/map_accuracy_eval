...
def auto_fullprocess(ref_dict,test_dict,test_img,verbose=True,visualize=True):
    ...
    ref_dict, test_dict, coverage = calc_coverage(ref_dict, test_dict, verbose=verbose)
    
    #2. Get the midpoints for each dict and align test set to the reference
    aligned_dict, xy_off, align_img = auto_align(ref_dict, test_dict, draw_img, verbose=verbose)
    if visualize:
        cv.imshow("Map Comparison",align_img)
        cv.waitKey(0)

    #3. Find the proper scale to minimize error metric
    scaled_dict, min_scale, scale_img = auto_scale(ref_dict,aligned_dict,align_img, verbose=verbose)
    if visualize:
    ...
    rot_dict, min_rot, rot_img = auto_rotate(ref_dict, scaled_dict, scale_img, verbose=verbose)
    if visualize:
...
def generate_scalefactor_plot(truth_dict,eval_dict,metrics,excludestd=0,image=None,save_file=None,title="Scale Factor Plot",units="px",note=""):
    ...
    hist, bins, _ = ax2.hist(scale, bins=100, edgecolor='black', alpha=1.0)
    max_value = max(hist)
    # Draw Exclusion Zone
...
if __name__ == "__main__":
    # main(sys.argv[1:])
    save_dir = "D:/map_accuracy_eval/output/draft_imgs"
    w_cm = 120
    # scale = get_scale_factor(745,w_cm*10)
    # pts1 = import_reference_pts("D:/map_accuracy_eval/example/nerve_basement_omron_scaled.pts")
    pts1 = import_reference_pts("D:/map_accuracy_eval/example/labyrinth_gt.pts")
    # pts1 = scale_dict(pts1,100)
    # bounds = get_dict_bounds(pts1)
    # pts1 = translate_dict(pts1,[0,-bounds[1][0]])

    # pts2 = import_reference_pts("D:/map_accuracy_eval/example/nerve_basement_spot_pixelcoord.txt")
    pts2 = import_reference_pts("D:/map_accuracy_eval/example/lab_e1.pts")
    # pts2 = scale_dict(pts2,0.2)
    # pts2 = translate_dict(pts2,[0,-bounds[1][0]])
    # pts2 = modify_dict(pts2)

    # map_image = cv.imread("D:/map_accuracy_eval/example/spot_map_image.png")
    map_image = cv.imread("D:/map_accuracy_eval/example/lab_e1.png")
    # map_image = scale_image_with_aspect_ratio(map_image,scale)
    map_name = "Labyrinth Example"
    ...
    # pts2, min_scale, _ = auto_scale(mtd,med)
    # mtd, min_scale, _ = auto_scale(med,mtd)
    # map_image = scale_image_with_aspect_ratio(map_image,min_scale)
    # point_errors, global_error, error_std = calc_error(mtd, pts2)
    metrics = calc_error(mtd, pts2)
    metrics.append(coverage)
    # fig_note = "Autoscaled eval to truth ({}) (W=120cm, Map=10Wx2W)".format(min_scale)
    fig_note = ""
    # global_error = round(global_error * scale,2)
    # error_std = round(error_std * scale,2)
    # for k in point_errors:
    #     point_errors[k] = point_errors[k] * scale

    # metrics = [coverage, global_error, error_std]
    
    # generate_contour_plot(mtd,pts2,point_errors,metrics,title=filename,image=map_image,
    #                       units="cm",note=fig_note)
    # generate_pointerror_contour_plot(mtd,pts2,metrics,title=filename,image=map_image,
    #                       units="px",note=fig_note)

    delta = 1.5
    inc = 0.5
    filename = "{}_scalefactor_exclude{:02d}".format(map_name,int(delta*10))  
    save_file = os.path.join(save_dir,filename)
    # generate_scalefactor_plot(mtd,pts2,metrics,title=filename,save_file=save_file,image=map_image,
    #                       units="px",note=fig_note)
    # print(metrics[1])
    # save_dir = "D:/map_accuracy_eval/output"


    # while delta <= 3:

    filename = "{}_scalefactor_exclude{:02d}".format(map_name,int(delta*10))   
    title = "{} Map Scale Factors - Excluding +/- {:.2f}stddev".format(map_name,delta)
    #     mtd, med, coverage = calc_coverage(pts1,pts3)

    #     # point_errors, global_error, error_std = calc_error(mtd, med)
    #     # pts2, min_scale, _ = auto_scale(mtd,med)
    #     # mtd, min_scale, _ = auto_scale(med,mtd)
    save_file = os.path.join(save_dir,filename)
    # generate_scalefactor_plot(mtd,pts2,metrics,excludestd=delta,title=title,save_file=save_file,image=map_image,
    #                     units="px",note=fig_note)
    ...
    # generate_scalefactor_plot(mtd,pts2,metrics,excludestd=delta,title=title,#image=map_image,
    #                     units="px",note=fig_note)
    ...
    #     point_errors, global_error, error_std = calc_error(mtd, pts3)
    #     # generate_contour_plot(mtd,pts2,point_errors,"D:/map_accuracy_eval/example/spot_map_image.png")
    #     errors.append(global_error)
    #     save_file = os.path.join(save_dir,filename)
    #     # generate_contour_plot(mtd,pts3,point_errors,title=filename)
    #     generate_contour_plot(mtd,pts3,point_errors,save_file=save_file,title=filename)
    # print(errors)